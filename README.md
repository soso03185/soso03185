## 👋 안녕하세요! soso03185입니다.

🎮 **유니티 게임 클라이언트 개발자**를 꿈꾸며 성장 중입니다.  
📱 캐주얼 게임과 러너 게임에 관심이 많으며, 퍼포먼스 최적화와 셰이더 연출에 재미를 느낍니다.  
💡 네트워크 구조, 프로토타이핑, Shader Graph, LINQ 등 다양한 기술들을 공부하고 있습니다.  
🎵 AI 음악도 제작하며, 전통과 현대를 잇는 감성적인 분위기의 곡을 실험하고 있습니다.

---

### 🛠 사용 기술 (Tech Stack)
- **언어**: C#, Python, JavaScript  
- **엔진 & 툴**: Unity (URP, Shader Graph), Git, GitHub, Firebase  
- **기타**: Suno AI, Midjourney, Figma, After Effects

---

### 📂 주요 프로젝트
- 🎓 **졸업작품 – 캐주얼 모바일 게임**  
  Google Play에 직접 출시  
  클라이언트 개발 전반 (UI, Firebase 연동, 콘텐츠 시스템, 최적화 등) 담당

- 🏃 **3D 타일 기반 러너 게임 (개발 중)**  
  곡선형 타일 경로, 2D 캐릭터 + 3D 배경, 실시간 타일 스포닝 및 셰이더 연출 적용

- 🎵 **AI 음악 채널 운영 중**  
  안예은 스타일의 전통 퓨전 발라드, 사이버펑크풍 일러스트와 함께 업로드 중

---

### 📫 연락처
- **이메일**: your.email@example.com  
- **Velog 블로그**: [velog.io/@yourid](https://velog.io/@yourid)  
- **LinkedIn**: [linkedin.com/in/yourname](https://linkedin.com/in/yourname)

---

### 💬 한 마디
> "기술은 수단이고, 유저에게 즐거움을 주는 것이 목표입니다."

